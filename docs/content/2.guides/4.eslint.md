...
Get instant feedback on broken links directly in your editor. The ESLint integration provides two rules that validate relative URLs against your app's known routes and sitemap data.
...
Use standard ESLint comments to skip individual lines:

```vue
<!-- eslint-disable-next-line link-checker/valid-route -->
<NuxtLink to="/special-case" />
```

## Setup with @nuxt/eslint

If you're using `@nuxt/eslint`, both rules are automatically registered. No configuration needed.
