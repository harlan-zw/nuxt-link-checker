...
The DevTools client now uses `nuxtseo-layer-devtools`, a shared layer used across all Nuxt SEO modules. This means the DevTools panel is now consistent across modules and maintained in one place. You only need to act if you were importing DevTools internals directly.
...
If you're using `@nuxt/eslint`, Nuxt Link Checker automatically registers both rules via the `eslint:config:addons` hook. Zero-config.
