...
export async function extractPayload(html: string, rootNodeId = '#__nuxt'): Promise<ExtractedPayload> {
  ...
  walkSync(ast, (node) => {
    ...
    // extract description
    if (node.type === ELEMENT_NODE && node.name === 'meta' && node.attributes?.name === 'description') {
      meta.description = node.attributes.content || ''
    }
    // get og image
...
async function lintHtmlFiles(filePaths: string[], eslintController: ReturnType<typeof createESLintWorker>) {
  ...
  await runParallel<string>(
    fileQueue,
    async (filePath) => {
      // Process one file at a time through the worker
      const lintResult = await eslintController.lintFiles([filePath])
      results.push({
        filePath,
        result: lintResult,
      })

      // Optional: free up memory explicitly after each file
      if (global.gc)
        global.gc()
    },
    {
      concurrency: 5, // Process 5 files at a time
      interval: 50, // Small delay between batches to allow GC
    },
  )
...
export function prerender(config: ModuleOptions, version?: string, eslintContainer: any, nuxt = useNuxt()) {
  ...
  nuxt.hooks.hook('nitro:init', async (nitro) => {
    console.log(nuxt.options.rootDir, nuxt.options.app.buildAssetsDir)
...
export function createNuxtAuditStorage(config: ModuleOptions, nuxt: Nuxt, nitro: Nitro) {
  const isBuild = nuxt.options._build && !nuxt.options.dev
  const storageFilepath = typeof config.report?.storage === 'string'
    ...
    : isBuild
      ...
      : `${nuxt.options.rootDir}/node_modules/.cache/nuxt/analyze`
